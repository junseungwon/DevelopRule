...
**핵심 원칙:**
1. 에이전트 정의(`.cursor/rules/agents/`)와 스킬(`.cursor/rules/skills/`)을 생성한다.
...
1. `프로젝트/.cursor/rules/agents/`, `프로젝트/.cursor/rules/skills/`, `프로젝트/CLAUDE.md`를 읽는다
...
**모든 에이전트는 반드시 `프로젝트/.cursor/rules/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 Agent 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
...
각 에이전트를 `프로젝트/.cursor/rules/agents/{name}.md`에 정의한다. 필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업. 에이전트 팀 모드에서는 `## 팀 통신 프로토콜` 섹션을 추가하여 메시지 수신/발신 대상과 작업 요청 범위를 명시한다.

> 정의 템플릿과 실제 파일 전문은 `references/agent-design-patterns.md`의 "에이전트 정의 구조" + `references/team-examples.md` 참조.

**CLAUDE.md 임시 동기화 (Phase 3 완료 시):**
Phase 3 완료 직후 CLAUDE.md의 에이전트 목록 테이블을 업데이트한다. 새 에이전트 추가 시 즉시 반영하고, 삭제/변경 시에도 동기화한다. 이것은 세션 중단에 대비한 **임시 동기화**이다 — Phase 5-4에서 전체 컨텍스트를 최종 확정한다.
...
각 에이전트가 사용할 스킬을 `프로젝트/.cursor/rules/skills/{name}/SKILL.md`에 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.
...
**CLAUDE.md 임시 동기화 (Phase 4 완료 시):**
Phase 4 완료 직후 CLAUDE.md에 스킬 목록과 디렉토리 구조를 업데이트한다. 새 스킬 디렉토리가 생성되면 디렉토리 트리에 즉시 반영한다. Phase 3과 마찬가지로 세션 중단 대비 **임시 동기화**이며, Phase 5-4에서 최종 확정한다.
...
**Step 1: 현황 감사**
- `.cursor/rules/agents/` 파일 목록 vs CLAUDE.md 에이전트 테이블 비교 → 불일치 목록 생성
- `.cursor/rules/skills/` 디렉토리 목록 vs CLAUDE.md 스킬 테이블 비교 → 불일치 목록 생성
- CLAUDE.md 디렉토리 구조와 실제 파일 시스템 비교 → 드리프트 감지
- 감사 결과를 사용자에게 보고한다
...
**Step 3: CLAUDE.md 동기화**
- 에이전트 테이블, 스킬 테이블, 디렉토리 구조, 변경 이력을 실제 상태에 맞게 갱신한다
- 변경 이력에 날짜, 변경 내용, 대상, 사유를 기록한다
...
- [ ] `프로젝트/.cursor/rules/agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
- [ ] `프로젝트/.cursor/rules/skills/` — 스킬 파일들 (SKILL.md + references/)
...
- [ ] **CLAUDE.md에 하네스 컨텍스트 등록** (에이전트 목록, 스킬 목록, 실행 규칙, 변경 이력)
- [ ] **CLAUDE.md에 에이전트/스킬/디렉토리 구조/주요 참조 경로 반영 완료** — Phase 3, 4 완료 시 즉시 동기화
