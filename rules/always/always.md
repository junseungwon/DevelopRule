...
모든 커밋 메시지는 아래 양식을 따른다. (에이전트 직접 커밋 포함)
...
| `#feat` | 새 기능 추가 |
| `#fix` | 버그 수정 |
| `#asset` | 에셋 추가/수정 (스프라이트, 사운드 등) |
| `#scene` | 씬 파일 변경 |
| `#refactor` | 동작 변경 없는 코드 정리 |
| `#perf` | 성능 개선 |
| `#docs` | 문서 변경 |
| `#chore` | 빌드 설정, Unity 버전, 패키지 변경 |
| `#revert` | 이전 커밋 롤백 |

- 제목은 **50자 이내**, **동사로 시작**, **마침표 없음**
- Co-Authored-By 라인은 사용자 요청 시에만 추가
- 전체 규칙 원본: `C:\DevelopRule\agents\Public\git-push.md`
